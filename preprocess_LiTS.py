...
class LITS_preprocess:
    ...
    def fix_data(self):
        ...
        for ct_file,i in zip(file_list,range(Numbers)):
            ...
            #print(sitk.GetArrayFromImage(new_ct).shape)
            if new_ct != None and new_seg != None:
                sitk.WriteImage(new_ct, self.fixed_path + 'ct/' + ct_file)
                sitk.WriteImage(new_seg, self.fixed_path + 'label/' + ct_file.replace('volume', 'segmentation').replace('.nii', '.nii.gz'))

    def process(self, ct_path, seg_path, classes=2):
    ...
    def write_name_list(self, name_list, file_name):
        ...
        for name in name_list:
            ct_path = self.fixed_path + 'ct/' + name
            seg_path = self.fixed_path + 'label/' + name.replace('volume', 'segmentation')
